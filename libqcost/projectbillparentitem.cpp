...
void ProjectBillParentItem::writeXml20(QXmlStreamWriter *writer) const{
    writer->writeStartElement( "Bills");
    for( QList<Bill *>::iterator i = m_d->billContainer.begin(); i != m_d->billContainer.end(); ++i ){
        (*i)->writeXml20( writer );
    }
...
void ProjectBillParentItem::readXml20(QXmlStreamReader *reader, ProjectPriceListParentItem * priceLists ) {
    while( (!reader->atEnd()) &&
           (!reader->hasError()) &&
           !(reader->isEndElement() && reader->name().toString().toUpper() == "BILLS") ){
        reader->readNext();
